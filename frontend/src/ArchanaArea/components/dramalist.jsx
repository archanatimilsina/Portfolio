...
const STATUS_COLOR = {
  ...
  Dropped: { bg: '#fdecea', color: '#6137a993', border: '#f5b7b1' },
  Faded: { bg: '#f9e1de', color: C.red, border: '#f5adee' }
};

const STATUS_ORDER = ['Watching', 'Plan to Watch', 'On Hold', 'Completed', 'Dropped'];
const STATUS_TABS = ['All', ...STATUS_ORDER];

const TYPE_COLOR = {
  drama: { bg: '#ede7f6', color: '#5e35b1', border: '#c5b4e8' },
  movie: { bg: '#fce4ec', color: '#c2185b', border: '#f48fb1' },
  anime: { bg: '#e0f7fa', color: '#00838f', border: '#80deea' },
  book: { bg: '#fff3e0', color: '#e65100', border: '#ffcc80' },
  manga: { bg: '#f1f8e9', color: '#558b2f', border: '#c5e1a5' },
};

// Softer per-type wash used behind each card, so anime/drama/movie/book/manga
// entries are recognizable at a glance without fighting with the badges above.
const TYPE_SURFACE = {
  drama: '#f7f4fd',
  movie: '#fdf1f6',
  anime: '#effbfc',
...
export default function Drama({ onBack }) {
  ...
  // Press "/" anywhere on the page to jump straight into search.
  useEffect(() => {
    const onKeyDown = (e) => {
  ...
  const handleDelete = async () => {
    ...
    setApiError('');
    try {
      const res = await fetch(`${BASE_URL}/watchlist/${delTarget.id}/`, {
        method: 'DELETE',
        next: { revalidate: 60 },
      });
  ...
  const visible = items
    ...
    .filter((x) => statusTab === 'All' || x.status === statusTab)
    .filter((x) => {
      const q = search.toLowerCase();
      return x.title.toLowerCase().includes(q) || x.genre.toLowerCase().includes(q);
    })
    .sort((a, b) => {
  ...
  const renderCard = (item) => (
    <Card key={item.id} $type={item.type} $busy={savingId === item.id}>
      <CardTopRow>
      ...
      <CardActions>
        <ActBtn onClick={() => openEdit(item)} disabled={savingId === item.id}>
          {savingId === item.id ? <BtnSpinner /> : '✏️'} Edit
        </ActBtn>
        <ActBtn $danger onClick={() => setDelTarget(item)} disabled={savingId === item.id}>
          🗑 Delete
        </ActBtn>
      </CardActions>
    </Card>
    ...
    <>
      ...
      <PageWrap>
        <TopBar>
          ...
          <TBRight>
            <PillBtn $primary onClick={openAdd}>
              + Add Entry
            </PillBtn>
        ...
        <Main>
          ...
          <ControlsRow>
            <SearchBox>
              ...
              />
              {search ? (
                <ClearSearchBtn onClick={() => setSearch('')} aria-label="Clear search">
                  ✕
                </ClearSearchBtn>
              ) : (
                <KbdHint>/</KbdHint>
          ...
          <StatusFilterRow>
            ...
            <FilterTabs>
              {STATUS_TABS.map((s) => (
                <FilterTab key={s} $active={statusTab === s} onClick={() => setStatusTab(s)}>
                  {s}
                </FilterTab>
              ))}
            </FilterTabs>
        ...
        {modal && (
          ...
          >
            <Modal>
              ...
              <ModalTitle>{modal === 'add' ? '➕ Add New Entry' : '✏️ Edit Entry'}</ModalTitle>
        ...
        {delTarget && (
          ...
          >
            <DeleteBox>
              ...
              <DeleteDesc>
                <strong style={{ color: C.dark }}>{delTarget.title}</strong> will be permanently
                removed. This cannot be undone.
              </DeleteDesc>
              <DeleteBtns>
                <DangerBtn onClick={handleDelete} disabled={savingId === delTarget.id}>
                  {savingId === delTarget.id ? (
                    <>
                      <BtnSpinner /> Removing...
                    </>
                  ) : (
...
const PillBtn = styled.button`
  ...
  letter-spacing: 1px;
  color: ${(p) => (p.$primary ? C.bg : C.dark)};
  background: ${(p) => (p.$primary ? C.dark : 'transparent')};
  ...
  gap: 0.4rem;
  &:hover {
    background: ${(p) => (p.$primary ? C.green : C.dark)};
    border-color: ${(p) => (p.$primary ? C.green : C.dark)};
    color: ${C.bg};
  }
`;
...
const SearchBox = styled.div`
  ...
  input {
    ...
    border: 1.5px solid ${C.border};
    border-radius: 100px;
    padding: 0.5rem 2.3rem 0.5rem 2.5rem;
...
const FilterTab = styled.button`
  ...
  background: ${(p) => (p.$active ? C.dark : C.white)};
  color: ${(p) => (p.$active ? C.bg : C.soft)};
  border: 1.5px solid ${(p) => (p.$active ? C.dark : C.border)};
  &:hover {
    border-color: ${C.dark};
    color: ${(p) => (p.$active ? C.bg : C.dark)};
  }
`;
...
const Card = styled.div`
  background: ${(p) => TYPE_SURFACE[p.$type] || C.white};
  border: 1.5px solid ${(p) => TYPE_COLOR[p.$type]?.border || C.border};
  border-left: 4px solid ${(p) => TYPE_COLOR[p.$type]?.color || C.border};
  border-radius: 18px;
  ...
  &:hover {
    ...
    box-shadow: 0 14px 36px rgba(26, 26, 46, 0.09);
    border-color: ${C.green};
  }
`;
...
const StatusBadge = styled.span`
  ...
  text-transform: uppercase;
  letter-spacing: 1px;
  background: ${(p) => STATUS_COLOR[p.$s]?.bg || C.muted};
  color: ${(p) => STATUS_COLOR[p.$s]?.color || C.soft};
  border: 1.5px solid ${(p) => STATUS_COLOR[p.$s]?.border || C.border};
...
const TypeBadge = styled.span`
  ...
  text-transform: uppercase;
  letter-spacing: 1px;
  background: ${(p) => TYPE_COLOR[p.$t]?.bg || C.muted};
  color: ${(p) => TYPE_COLOR[p.$t]?.color || C.soft};
  border: 1.5px solid ${(p) => TYPE_COLOR[p.$t]?.border || C.border};
  padding: 0.2rem 0.6rem;
  border-radius: 100px;
`;
...
const ActBtn = styled.button`
  ...
  text-transform: uppercase;
  transition: all 0.18s ease;
  background: ${(p) => (p.$danger ? C.redLt : C.muted)};
  color: ${(p) => (p.$danger ? C.red : C.dark)};
  border: 1.5px solid ${(p) => (p.$danger ? '#f5b7b1' : C.border)};
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: 0.35rem;
  &:hover:not(:disabled) {
    background: ${(p) => (p.$danger ? C.red : C.dark)};
    color: ${C.bg};
    border-color: ${(p) => (p.$danger ? C.red : C.dark)};
  }
  &:disabled {
    opacity: 0.45;
    cursor: not-allowed;
